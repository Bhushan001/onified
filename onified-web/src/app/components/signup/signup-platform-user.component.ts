...
@Component({
  ...
  templateUrl: './signup-platform-user.component.html',
  styleUrls: ['./signup-platform-user.component.scss'],
  standalone: false
})
export class SignupPlatformUserComponent implements OnInit, OnDestroy {
  ...
  onSubmit() {
    ...
    this.authService.registerPlatformUser(registerData).subscribe({
      next: (result) => {
        if (result.success) {
