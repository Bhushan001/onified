...
export class AuthGuard implements CanActivate {
  ...
  private isMicroFrontendMode(): boolean {
    try {
      return window.location.pathname.includes('/host/workspace') || 
             window.location.href.includes('localhost:4500');
    } catch {
      return false;
    }
  }
  ...
  private shouldUserBeInThisPortal(roles: string[]): boolean {
    // Workspace portal is for Tenant Admins
    return roles && roles.includes('PLATFORM.Management.User');
  }
  ...
  private redirectToAppropriatePortal(roles: string[]): void {
    if (roles && roles.includes('PLATFORM.Management.Admin')) {
      ...
      window.location.href = 'http://localhost:4400'; // Workspace app URL
